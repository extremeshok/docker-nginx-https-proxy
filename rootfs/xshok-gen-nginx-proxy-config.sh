    ...
    if [[ $PROXY_DOMAINS =~ [\,\;] ]]; then
        ...
        for proxy_domain in $proxy_domain_array ; do
            ...
            if [[ ! -z "${domain}" ]] && [[ ! -z "${proxy}" ]]; then
                if [[ -f "/certs/${domain}/fullchain.pem" ]] && [[ -f "/certs/${domain}/privkey.pem" ]] && [[ -f "/certs/${domain}/chain.pem" ]] ; then
                    ...
                    if [[ ! -f "/certs/${domain}/cefullchainrt.pem" ]] || [[ ! -f "/certs/${domain}/privkey.pem" ]] ; then
